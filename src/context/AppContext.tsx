...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [data, setData] = useState<AppData>(() => {
    if (typeof window !== 'undefined') {
      try {
        const storedVersion = localStorage.getItem('akardaya_app_version');
        const saved = localStorage.getItem('akardaya_app_data');
        
        if (saved) {
          const parsed = JSON.parse(saved);
          if (parsed && typeof parsed === 'object') {
            const normalized = safeNormalizeData(parsed);

            // Auto-upgrade stale cache to new version specifications
            if (storedVersion !== APP_DATA_VERSION) {
              console.info(`[AppEngine] Upgrading local cache version from '${storedVersion}' to '${APP_DATA_VERSION}'`);
              localStorage.setItem('akardaya_app_version', APP_DATA_VERSION);
              localStorage.setItem('akardaya_app_data', JSON.stringify(normalized));
              if (!localStorage.getItem('akardaya_spreadsheet_url')) {
                localStorage.setItem('akardaya_spreadsheet_url', PERMANENT_GAS_URL);
              }
            }
            return normalized;
          }
        } else {
          // Fresh install in browser
          localStorage.setItem('akardaya_app_version', APP_DATA_VERSION);
          localStorage.setItem('akardaya_app_data', JSON.stringify(INITIAL_APP_DATA));
          localStorage.setItem('akardaya_spreadsheet_url', PERMANENT_GAS_URL);
        }
      } catch (e) {
        console.warn('Error reading or upgrading localStorage cache:', e);
      }
    }
    return INITIAL_APP_DATA;
  });
